...
// 官方 bundle 形态：exports {name, inject, apply} 经 __ModuleLoader__.load 注册，
// 由 client 内核挂载时调用 apply(ctx)（见文件尾注释）。宠物本体零平台模块依赖
// （CSS 内联注入，动画/拖拽/菜单全部自建）；唯一平台依赖是设置面板卡片
// （settings.plugin.item 槽渲染方是 React——react 由平台种子表提供，
// esbuild --external:react，见 scripts/build-client.mjs）。
//
...
// 官方设置面板卡片（settings.plugin.item keyed 槽，key = 设置命名空间 whale-girl）：
// 可选增强——服务缺失/注册失败时宠物照常跑，仅无卡片（见 apply 内守卫）。
import { registerWhaleSettingsCard } from './settings-card.mjs'
...
// 设置命名空间（= 设置面板卡片 key，须与 Node half 的 settings.register 同名否则卡片
// 永不出现）：client bundle 不可 import src/config.mjs（其 import schemastery，浏览器
// 不打包），此处字面量声明——改 Node half 命名空间时此字面量必须同步（交叉契约，
// 注释 + 设置面板实测守护）。
const SETTINGS_NAMESPACE = 'whale-girl'
...
export function apply(ctx = {}) {
  ...
  // 设置面板卡片（可选增强）：inject 声明 slots/locale/settingsScope 后才可取到服务
  // （见文件尾 inject 导出）；任一缺失或注册抛错都不阻断宠物本体，仅无卡片。
  ...
  // 交互开始时宠物视觉上是否处于 sleep 动画（pointerdown/keydown 捕获）——wake 醒觉
  // 判定绑定视觉状态（非 sleeping 派生变量：会话活跃时视觉已离开 sleep 但 sleeping 仍
  // true，那时交互不播 wake，见 logic.mjs wakeFromInteraction）。
  ...
  const setState = (name) => {
    ...
    // 帧 0 的应用时刻（不是 0）：否则本 tick 的推进条件 now-0>=1000/fps 恒真，
    // 进入状态的同 tick 立即跳到帧 1——首帧从不绘制（wake 两帧 once 因此只播末帧）。
    // 帧 0 至少展示一个帧间隔，之后按 fps 推进（见 issue #12 后手测发现的 wake 单帧 bug）。
  ...
  const loadAssets = async (attempt = 1) => {
    try {
      // manifest 例外 immutable 缓存：assets 路由对全部资源下发 immutable（素材契约，
      // 见 2026-08-16-assets-cache-idle-first-load.md），但 manifest 是索引、会随发布
      // 变化（如 frameMs/角色 meta）——以 no-store 每次页面加载重新拉取（小文件，
      // assets 路由按请求读磁盘）；sheet 仍走 immutable 缓存（改图改名契约不变）。
      ...
      // 角色尺寸：meta.stageSize 作为 --pet-size 默认（仅用户从未配置 size 时生效；
      // 用户配置 size 是权威——sizeConfigured 在 boot/refresh 的 applyClientConfig
      // 同步置位，不再用 lastConfigRevision 守卫：它只在 refresh 更新，与这里的
      // manifest 拉取并发，会把已应用的配置 size 覆盖回 stageSize（issue #12）。
  ...
  const switchCharacter = async (id) => {
    ...
    try {
      ...
      // 与 loadAssets 同守卫——sizeConfigured 置位后不再覆盖，见 issue #12）。
      ...
      frame = 0
      lastFrameAt = Date.now() // 同 setState：换肤后帧 0 先可见一帧间隔再推进
    } catch {
      // 预加载失败：保留当前角色
    }
  }
  ...
  const tick = () => {
    ...
    if (cfg && loaded.has(sheetKey(cfg.sheet))) {
      ...
      if (!showingSprite) {
        ...
        showingSprite = true
        frame = 0
        lastFrameAt = Date.now() // 同 setState：换肤首帧先可见一帧间隔再推进（帧 0 不被跳过）
      }
      ...
      // （否则会推进到 -width 位置闪空白）。
      // 逐帧停留时长（frameMs，可选）：frameMs[i] = 帧 i 的停留 ms，缺省回退统一 fps 节拍——
      // 「首帧稍长、后续稍短」的缓动由 manifest 数据表达，播放器不特判状态名。
      const holdFor = cfg.frameMs?.[frame] ?? 1000 / cfg.fps
      if (cfg.frames > 1 && now - lastFrameAt >= holdFor) {
  ...
  // 若宠物视觉上处于 sleep 动画（交互开始时捕获的 interactFromSleep）则附加 wake 过渡
  // （「被拖起来」的自然醒觉）。薄执行：决策在纯函数。
  ...
  // sizeConfigured：用户是否显式配置过 size（applyClientConfig 同步置位）——stageSize
  // 兜底只在「从未配置过 size」时生效。曾用 lastConfigRevision===0 作守卫，但它只在
  // refresh() 更新，与 loadAssets 的 manifest 拉取并发形成竞态（issue #12：boot 已
  // 应用 160px 后被 stageSize 覆盖回 110px，首个 /state 到达后才跳回 160px）。
  ...
  const applyClientConfig = (config) => {
    ...
    if (typeof config.size === 'number') {
      sizeConfigured = true // 用户显式配置 size：此后 stageSize 兜底不再覆盖（issue #12）
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (act !== null && typeof act === 'object') {
        ...
        armWorking() // 会话活跃状态变化 → 武装/撤防 working 插曲（守卫保证不重排待触发决策）
      }
  ...
  hitarea.addEventListener('pointerdown', (e) => {
    ...
    dragging = false
    moved = false
    interactFromSleep = animState === 'sleep' // 交互开始时刻的视觉状态（wake 判定基准）
    stopWalk() // 被拖走即停下游走
    startX = e.clientX
    startY = e.clientY
  ...
  stage.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault()
      interactFromSleep = animState === 'sleep' // 键盘开菜单同样按交互开始时刻判定醒觉
      toggleMenu()
    }
  })
  ...
  // 由 refresh 在会话开始时重新武装）。
  // 守卫：已有待触发决策时不再重排——refresh 每 pollMs 调用本函数，若每次清掉重排，
  // 12-30s 的随机等待会被无限推后，working 永不激活（手测：working 动画不播放）。
  const armWorking = () => {
    ...
    if (workingTimer !== null) return // 已有待触发决策：不重置（防止 refresh 重排推后激活）
  ...
  const dispose = () => {
    ...
    if (offSettings !== null) { offSettings(); offSettings = null }
    host.remove()
    style.remove()
  }
  return dispose
...
// 注册，由 client 内核挂载时调用 apply(ctx)（cordis 条目即本模块导出面）。Pet 本体
// 只依赖 DOM；inject 声明的服务仅供设置面板卡片（settings.plugin.item 槽需要
// slots/locale/settingsScope——web 组合恒有；声明进来后 apply 内守卫使用，任一
// 缺失或注册失败仅「无卡片」，宠物照常跑）。
