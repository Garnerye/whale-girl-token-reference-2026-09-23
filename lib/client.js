...
var WhaleSettingsForm = class {
  ...
  constructor(scope, defaults) {
    ...
    __publicField(this, "getSnapshot", () => {
      ...
      if (this.cache !== void 0 && this.cache.snap === snap && this.cache.draft === this.draft && this.cache.saving === this.saving && this.cache.failed === this.failed) {
      ...
      for (const def of CARD_FIELDS) {
        ...
        fields[def.path] = fromCommitted !== void 0 ? fromCommitted : leafOf(this.defaults, def.path);
      }
      const state = {
    ...
    __publicField(this, "edit", (path, value) => {
...
function WhaleSettingsCard(props) {
  ...
  const t = props.t;
  const row = (def, index) => {
    const control = def.kind === "toggle" ? el(Switch, { checked: state.fields[def.path] === true, disabled, label: t(def.labelKey), onChange: (checked) => {
      props.edit(def.path, checked);
  ...
  return el(
    ...
    el(
      ...
      el(
        "span",
        { style: { flex: "none", display: "flex", color: "var(--dsw-alias-label-tertiary)", transition: "transform .16s", transform: open ? "rotate(180deg)" : "none" } },
    ...
    open ? el(
      "div",
      { style: { borderTop: "1px solid var(--dsw-alias-border-l2)", margin: "0 16px", paddingBottom: 8 } },
      ...
      el(
        "div",
        { style: { display: "flex", flexDirection: "column", padding: "12px 0", gap: 0 } },
        CARD_FIELDS.map((def, index) => row(def, index))
      ),
      el(
        "div",
        { style: { display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, padding: "12px 0 4px", borderTop: "1px solid var(--dsw-alias-border-l2)" } },
