...
- 已知边界：localStorage 偏好由用户手动设置（无 UI，P2 加菜单「换角色」）；角色 meta.stageSize 已接入 `--pet-size` 兜底——权威语义（用户配置 size 优先、stageSize 仅作未配置时的默认）见 [bug-fix/2026-08-20-stage-size-config-race.md](../bug-fix/2026-08-20-stage-size-config-race.md)（issue #12 竞态修复）；~~think/wait 仍 emoji 兜底~~（470b940 起已有真 sheet：think.png/wait.png，1 帧 + float/wiggle）。
