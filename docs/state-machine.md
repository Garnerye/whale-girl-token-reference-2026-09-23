...
| `idle`（缓冲） | 拖拽放下后 1.5s（`dragReleaseUntil`） | client 本地 | 放下缓冲，再进底层状态；睡着被拖起时让位 `wake` |
...
- **用户交互醒觉**：拖拽放下/喂食/玩耍/开菜单都是用户在场信号——空闲计时从交互时刻重新起算（`wakeFromInteraction`），交互后保持清醒直至再次空闲 ≥ sleepAfterMs；交互瞬间若宠物**视觉上处于 sleep 动画**（交互开始时刻 `animState === 'sleep'`）则附加 wake 醒觉过渡（拖拽放下直接播 wake——放下缓冲让位醒觉过渡；醒着拖拽仍先 1.5s idle 缓冲）。醒觉判定绑定视觉 sleep 状态，不用 `sleeping` 派生变量（会话活跃时视觉已离开 sleep 但 `sleeping` 仍 true，此时交互不播 wake）。
